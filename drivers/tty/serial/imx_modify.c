	...
	if (port->rs485.flags & SER_RS485_ENABLED &&
        ...
        if (sport->txen_gpio != -1) {
            if (port->rs485.flags & SER_RS485_RTS_AFTER_SEND){
				//printk("*** <kernel> stop tx...\n");
                gpio_set_value(sport->txen_gpio, 0);
                gpio_set_value(sport->rdm_gpio_rx, 0);
            }
        } else {
	...
	if (port->rs485.flags & SER_RS485_ENABLED) {
        ...
        if (sport->txen_gpio != -1) {
            if (port->rs485.flags & SER_RS485_RTS_ON_SEND){
				//printk("*** <kernel> start tx...\n");
                gpio_set_value(sport->txen_gpio, 1);
                gpio_set_value(sport->rdm_gpio_rx, 1);
            }
        } else {
		...
		struct platform_device *pdev)
{
    ...
    if (of_get_property(np, "fsl,rs485-gpio-txen", NULL))
        ...
        sport->txen_gpio = -1;
	printk("*** <kernel> rs485-gpio-txen: %d\n", sport->txen_gpio);
	...
	/* add by star for dmx512 gpio */
	/* rdm_gpio_rx */
	if(of_get_property(np, "pavt,rdm-gpio-rx", NULL))
		sport->rdm_gpio_rx = of_get_named_gpio(np, "pavt,rdm-gpio-rx", 0);
	else
		sport->rdm_gpio_rx = -1;
	printk("*** <kernel> rdm_gpio_rx: %d\n", sport->rdm_gpio_rx);

	if(gpio_is_valid(sport->rdm_gpio_rx))
		devm_gpio_request_one(&pdev->dev, sport->rdm_gpio_rx, GPIOF_OUT_INIT_HIGH, "rdm-rx");
	else
		sport->rdm_gpio_rx = -1;

	/* rdm_gpio_tx */
	if(of_get_property(np, "pavt,rdm-gpio-tx", NULL))
		sport->rdm_gpio_tx = of_get_named_gpio(np, "pavt,rdm-gpio-tx", 0);
	else
		sport->rdm_gpio_tx = -1;
	printk("*** <kernel> rdm_gpio_tx: %d\n", sport->rdm_gpio_tx);

	if(gpio_is_valid(sport->rdm_gpio_tx))
		devm_gpio_request_one(&pdev->dev, sport->rdm_gpio_tx, GPIOF_OUT_INIT_LOW, "rdm-tx");
	else
		sport->rdm_gpio_tx = -1;
