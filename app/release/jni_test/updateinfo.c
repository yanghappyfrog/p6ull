...
int get_elec_data(int elec_ch)
{
	...
	char channel[2] = {0};
	//char w_buf[128] = {0};
	char w_buf[512] = "<AT_CHECK0_ELECT_[A001_001][A002_002][A003_003][B004_004][B005_005][B006_006][C007_007][C008_008][C009_009][A010_010][A011_011][A012_012]>";
	char r_buf[512] = {0};
...
int get_power_data(int power_ch)
{
	int i, ret;
	char w_buf[128] = {0};
	char r_buf[128] = {0};
...
/* 获取温湿度 */
int get_tmp_data(int tmp_ch)
{
	int i, ret = 0, ch = 0;
	char *p;
	char w_buf[128] = {0};
	char r_buf[512] = "<AT_T/H_1234/1234>";
	//char r_buf[512] = {0};
	char temp[512] = {0};
	char key[24] = {0};
	char vals[2][32] = {0} ;
	...
	FILE *air_filep;
    dictionary * ini ;
    ini = iniparser_load(TMP_DATA_FILE);
	
	bzero(w_buf, sizeof(w_buf));
	sprintf(w_buf, "<AT_READ>");
	printf("send data: %s\n", w_buf);
	//rs485_write(tmp_ch, w_buf);

	//bzero(r_buf, sizeof(r_buf));
	//rs485_read(tmp_ch, r_buf, 2);
	printf("recv data: %s\n", r_buf);

	ret = strlen(r_buf);
	if(ret != 18){
		printf("tmp data[%d] error!\n", ret);
		return -1;
	}
...
int get_channle()
{
	...
	for(i = 1; i < 5; i++)
	{
		...
		if(ret == 0){
			printf("ex%d device is SCANQRCODE!\n", channel);
			get_id_data(channel);
			continue;
		}
		...
		ret = strncmp(str, "TMP", strlen("TMP"));
		if(ret == 0){
			printf("ex%d device is tempreture!\n", channel);
			get_tmp_data(channel);
			continue;
		}
	}
