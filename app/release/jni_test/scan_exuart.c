...
//	printf("*** %s *** \n", __func__);
	...
	ret = strncmp(r_buf, "<AIRSWITCH_DATA_OK>", strlen("<AIRSWITCH_DATA_OK>"));
	if(ret == 0){
		sprintf(key, "extend_uart:ex%d", channel);
		printf("AIRSWITCH get ok[%s]!!\n", key);
		iniparser_set(ini, key, AIR);
		return 0;
	}
	ret = strncmp(r_buf, "<AT_EMCM_OK>", strlen("<AT_EMCM_OK>"));
	if(ret == 0){
		sprintf(key, "extend_uart:ex%d", channel);
		printf("tempreture get ok[%s]!!\n", key);
		iniparser_set(ini, key, TMP);
		return 0;
	}
